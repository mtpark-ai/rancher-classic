...
func (h *StatsHandler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if ok, _ := authToken.Claims.(jwt.MapClaims)["payload"].(bool); ok {
...
func getProjectOrService(token *jwt.Token) ([]map[string]string, error) {
	data, ok := token.Claims.(jwt.MapClaims)["project"]
	if !ok {
		data, ok = token.Claims.(jwt.MapClaims)["service"]
	}
	if ok {
...
func (h *StatsHandler) extractHostUUID(token *jwt.Token) (string, bool) {
	hostUUID, found := token.Claims.(jwt.MapClaims)["hostUuid"]
	if !found {
